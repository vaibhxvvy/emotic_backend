...
# ------------------------------
# CORS - allow origins configurable via env var
# ------------------------------
default_origins = [
    "http://127.0.0.1:5500",
    "http://127.0.0.1:5000",
...
def demo_guess(text: str) -> str:
    ...
    if any(w in t for w in ["surpris","wow","shocked","omg"]): return "surprise"
    if any(w in t for w in ["disgust","gross","yuck"]): return "disgust"
    return "neutral"

# ------------------------------
# Serve SPA and static files (catch-all)
# ------------------------------
@app.route("/", defaults={"path": ""})
@app.route("/<path:path>")
def serve_spa(path):
    """
    Serve static files if they exist. Otherwise return static/index.html so the
    client-side router (React Router) can handle routing.
    """
    static_dir = app.static_folder or "static"
    requested = path or "index.html"
    candidate = os.path.join(static_dir, requested)
    ...
    return jsonify({
        "error": "index.html not found in static/. Did you build the frontend and copy its dist into the backend static folder?"
    }), 500
